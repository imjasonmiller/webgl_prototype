...
function UnrealBloomPass(resolution, strength, radius, threshold) {
  if (!(this instanceof UnrealBloomPass)) {
  ...
  for (var i = 0; i < this.nMips; i++) {
    this.seperableBlurMaterials.push(
      this.getSeperableBlurMaterial(kernelSizeArray[i]),
    )
    this.seperableBlurMaterials[i].uniforms["texSize"].value = new Vector2(
      resx,
      resy,
    )
    resx = Math.round(resx / 2)
...
UnrealBloomPass.prototype.render = function(
  renderer,
  writeBuffer,
  readBuffer,
  delta,
  ...
  for (var i = 0; i < this.nMips; i += 1) {
    this.quad.material = this.seperableBlurMaterials[i]

    this.seperableBlurMaterials[i].uniforms.colorTexture.value =
      inputRenderTarget.texture
    this.seperableBlurMaterials[i].uniforms.direction.value =
